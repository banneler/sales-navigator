...
function buildElevatorShell(floorName, innerHtml) {
  ...
  return (
    `<div class="elevator-reveal-wrap group/elev mb-8 w-full">` +
      ...
      posterL +
      `<div class="elevator-reveal-card [container-type:inline-size] group relative w-full max-w-[min(100%,22rem)] shrink min-w-0 cursor-pointer aspect-square overflow-hidden rounded-xl border-[3px] border-slate-600 bg-gradient-to-b from-slate-600 via-slate-800 to-slate-950 p-2 shadow-[0_8px_32px_rgba(15,23,42,0.45)] ring-1 ring-slate-900/60 sm:max-w-[min(100%,24rem)] xl:max-w-lg 2xl:max-w-xl">` +
