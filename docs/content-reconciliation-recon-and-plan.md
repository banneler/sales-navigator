...
- Bring ABM (3), OBR (3), iTV (4) up to ~10 KCs each, **with each new KC tied to a specific line in a specific PDF anchor**.
- Bring UC down from 15 → ~10 by **merging or cutting** the lowest-yield questions (likely the duplicates around Embedded vs Direct Routing).
- Confirm every other module's KC explanations cite the document, not generic prose.

**Output:** Catalog-wide KC density at the rule target (~10), with provable explanations.
